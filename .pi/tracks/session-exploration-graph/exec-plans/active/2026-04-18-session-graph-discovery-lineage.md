...
- [~] (2026-04-18 16:20 UTC) Tightened readability further: grouped row assignment is now segmented into first-seen timeline bands so turn 0 concepts stay above turn 1, turn 1 above turn 2, etc., while intra-band anchor sorting still keeps related search → action → file chains close. Added stronger selection contrast by dimming non-highlighted nodes and edges when any graph node is selected.
