...
export async function get_project_file_stats(
  ...
  const tool_distribution: NameCount[] = Array.from(tool_map.entries())
  ...
  for (const s of project_sessions) {
    for (const [k, v] of Object.entries(s.read_files)) {
      ...
      if (!set) { set = new Set(); file_sessions.set(k, set); }
      set.add(s.id);
    }
  ...
  const directory_stats: DirectoryStat[] = Array.from(all_dirs).map((dir_path) => {
  ...
  const activity_by_date: DayCount[] = Array.from(date_map.entries())
  ...
  const file_insights: FileInsightRecord[] = Array.from(all_file_paths).map((file_path) => {
    ...
    return { path: file_path, read_count, edit_count, write_count, total_count, distinct_session_count };
  });
