...
export async function get_analytics_overview(
  ...
  const avg_turns_per_session = total_sessions > 0
    ? total_turns / total_sessions
    : 0;
  const avg_session_duration_seconds = duration_count > 0
  ...
  for (const session of all_sessions) {
    ...
    if (dt !== null) {
      ...
      cost_by_date_map.set(date, (cost_by_date_map.get(date) ?? 0) + session.total_cost);
    }
  }

  const sessions_by_date: DayCount[] = Array.from(sessions_by_date_map.entries())
    .map(([date, count]) => ({ date, count }));
  sessions_by_date.sort((a, b) => a.date.localeCompare(b.date));

  const cost_by_date: DayCost[] = Array.from(cost_by_date_map.entries())
    .map(([date, cost]) => ({ date, cost }));
  cost_by_date.sort((a, b) => a.date.localeCompare(b.date));

  // Aggregate models by (model_id, provider) pair
  const model_map = new Map<string, { message_count: number; total_cost: number }>();
  for (const session of all_sessions) {
  ...
  const models: ModelAggregate[] = Array.from(model_map.entries()).map(([key, val]) => {
    const [model_id, provider] = key.split("\0");
    return { model_id, provider, message_count: val.message_count, total_cost: val.total_cost };
  });

  // Aggregate tools
  const tool_map = new Map<string, { total_calls: number; total_errors: number }>();
  for (const session of all_sessions) {
    for (const [tool_name, tool_summary] of Object.entries(session.tool_calls)) {
      const entry = tool_map.get(tool_name) ?? { total_calls: 0, total_errors: 0 };
      entry.total_calls += tool_summary.calls;
      entry.total_errors += tool_summary.errors;
      tool_map.set(tool_name, entry);
    }
  }
  const tools: ToolAggregate[] = Array.from(tool_map.entries()).map(([name, val]) => ({
    name,
  ...
  for (const session of all_sessions) {
    for (const [command, count] of Object.entries(session.bash_commands)) {
      bash_commands_map.set(command, (bash_commands_map.get(command) ?? 0) + count);
    }
  ...
  const make_top_20 = (m: Map<string, number>): NameCount[] => {
    const arr: NameCount[] = Array.from(m.entries())
