...
export async function get_time_breakdown(
  ...
  const avg_cost_per_session = total_sessions > 0
  ...
  const by_weekday: WeekdayStat[] = DAY_NAMES.map((day, i) => {
    const share = total_sessions > 0
      ? (weekday_sessions[i] / total_sessions) * 100.0
      : 0.0;
    return { day, sessions: weekday_sessions[i], cost: weekday_cost[i], share };
  });

  const by_time_of_day: TimeOfDayStat[] = TIME_BUCKETS.map(([label, hour_start, hour_end], i) => {
    const share = total_sessions > 0
      ? (tod_sessions[i] / total_sessions) * 100.0
      : 0.0;
    return { label, hour_start, hour_end, sessions: tod_sessions[i], cost: tod_cost[i], share };
  });

  const daily_sessions: DayCount[] = Array.from(daily_sessions_map.entries())
    .map(([date, count]) => ({ date, count }));
  daily_sessions.sort((a, b) => a.date.localeCompare(b.date));

  const daily_cost: DayCost[] = Array.from(daily_cost_map.entries())
