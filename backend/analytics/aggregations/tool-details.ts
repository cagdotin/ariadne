...
function get_tool_source(
	session: SessionSummary,
	tool_name: string,
): Record<string, number> | null {
	switch (tool_name) {
		case "bash":
			return session.bash_commands;
		case "read":
			return session.read_files;
		case "edit":
			return session.edit_files;
		case "write":
			return session.write_files;
		default:
			return null;
	}
}
...
export async function get_tool_details(
	...
	const items: NameCount[] = Array.from(items_map.entries()).map(
	...
	for (const s of filtered) {
		...
		for (const [key, count] of Object.entries(source)) {
			...
			project_calls_map.set(
				s.project_path,
				(project_calls_map.get(s.project_path) ?? 0) + count,
			);
		}
	}

	const by_project: ProjectToolSummary[] = Array.from(
		project_items_map.entries(),
	).map(([ppath, imap]) => {
		let pitems: NameCount[] = Array.from(imap.entries()).map(
			([name, count]) => ({ name, count }),
		);
	...
	const by_date: DayCount[] = Array.from(date_map.entries()).map(
