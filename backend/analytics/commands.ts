...
import { list_projects, get_file_sizes } from "./helpers.js";
...
register_handler("get_analytics_overview", async (payload) => {
  const project_path = typeof payload.projectPath === "string"
    ? payload.projectPath
    : null;
  const range_days = typeof payload.rangeDays === "number"
    ? payload.rangeDays
    : 0;
