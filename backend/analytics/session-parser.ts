						...
						if (b.type === "toolCall") {
							...
							if (tool_name === "bash") {
								...
								if (typeof cmd === "string") {
									const program = cmd.trim().split(/\s+/)[0] ?? cmd;
								...
								if (typeof p === "string") {
									summary.read_files[p] = (summary.read_files[p] ?? 0) + 1;
								}
							} else if (tool_name === "edit" || tool_name === "Edit") {
								const p = arguments_obj?.path;
								if (typeof p === "string") {
									summary.edit_files[p] = (summary.edit_files[p] ?? 0) + 1;
								}
							} else if (tool_name === "write" || tool_name === "Write") {
								const p = arguments_obj?.path;
								if (typeof p === "string") {
									summary.write_files[p] = (summary.write_files[p] ?? 0) + 1;
								}
							}
						}
					}
