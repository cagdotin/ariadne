...
export async function fetch_codex_limits(): Promise<ProviderLimitSnapshot> {
	try {
		...
		console.error(
			`[provider_limits::codex] app-server probe failed: ${rpc_err}`,
		);
		try {
			return await fallback_session_logs();
		} catch (log_err) {
			console.error(
				`[provider_limits::codex] session log fallback failed: ${log_err}`,
			);
...
async function run_rpc_exchange(
	...
	// biome-ignore lint/style/noNonNullAssertion: stdio configured as pipe
	...
	rl.on("line", (raw_line: string) => {
		...
		if (parsed.error) {
			entry.reject(
				new Error(`codex RPC error: ${JSON.stringify(parsed.error)}`),
			);
			return;
		}
	...
	if (!limits_result?.rateLimits) {
		throw new Error(
			"Failed to parse rateLimits response: missing rateLimits field",
		);
	}
	...
	if (snap.secondary) {
		windows.push(
			normalize_window("secondary", "Secondary (7d)", snap.secondary),
		);
	}
	...
	const plan_type = snap.planType ?? account_result?.account?.planType ?? null;
...
export function make_error_snapshot(
