...
export function qmd_list_indexes(): QmdIndex[] {
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      try {
        const cc_row = db.prepare("SELECT COUNT(*) as cnt FROM store_collections").get() as
          | { cnt: number }
          | undefined;
...
export function qmd_get_status(index: string): QmdStatus {
  ...
  try {
    ...
    const total_documents =
      (db.prepare("SELECT COUNT(*) as cnt FROM documents").get() as { cnt: number } | undefined)
        ?.cnt ?? 0;

    const active_documents =
      (
        db.prepare("SELECT COUNT(*) as cnt FROM documents WHERE active = 1").get() as
          | { cnt: number }
          | undefined
      )?.cnt ?? 0;
    ...
    try {
      const ts_row = db
        .prepare("SELECT MAX(modified_at) as ts FROM documents WHERE active = 1")
      ...
      if (ts) {
        ...
        if (!isNaN(parsed.getTime())) {
          ...
          days_since_update = Math.max(0, Math.floor(diff_ms / (1000 * 60 * 60 * 24)));
        }
      }
    } catch {
      // ignore
...
export function qmd_get_collection_detail(index: string, name: string): QmdCollectionDetail {
  const db = open_db(index);
  try {
    const coll_sql =
...
function get_collection_documents(db: Database.Database, collection: string): QmdDocument[] {
  const sql =
...
export function qmd_get_indexed_paths(index: string, collection: string): string[] {
  const db = open_db(index);
  try {
    const rows = db
