...
import { send_request, on_backend_event } from "./backend-supervisor.js";

export function setup_ipc_router(): void {
  // Generic command channel: renderer invokes "ariadne:command" with { channel, payload }
  ipcMain.handle(
    "ariadne:command",
    async (_event, args: { channel: string; payload: Record<string, unknown> }) => {
  ...
  ipcMain.handle(
    "ariadne:dialog",
    async (_event, args: { type: string; options?: Record<string, unknown> }) => {
