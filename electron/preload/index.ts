...
function invoke_command(channel: string, payload: Record<string, unknown>): Promise<unknown> {
  return ipcRenderer.invoke("ariadne:command", { channel, payload });
}

// Subscription bookkeeping for events.on / events.off
const subscription_listeners = new Map<string, (event: Electron.IpcRendererEvent, data: { event: string; payload: unknown }) => void>();
...
contextBridge.exposeInMainWorld("ariadne", {
  ...
  commands: {
    ...
    analytics: {
      list_projects: () =>
        invoke_command("list_projects", {}),

      get_analytics_overview: (payload: { projectPath?: string | null; rangeDays?: number | null }) =>
      ...
      get_all_sessions: (payload: { projectPath?: string | null; rangeDays?: number | null }) =>
        invoke_command("get_all_sessions", payload),

      resync_sessions: () =>
        invoke_command("resync_sessions", {}),

      get_project_file_stats: (payload: { projectPath: string; rangeDays?: number | null }) =>
        invoke_command("get_project_file_stats", payload),

      get_time_breakdown: (payload: { rangeDays: number; projectPath?: string | null }) =>
      ...
      get_tool_details: (payload: { toolName: string; projectPath?: string | null; rangeDays?: number | null }) =>
    ...
    qmd: {
      qmd_list_indexes: () =>
        invoke_command("qmd_list_indexes", {}),
      ...
      qmd_add_collection: (payload: { index: string; name: string; path: string; pattern?: string | null }) =>
      ...
      qmd_rename_collection: (payload: { index: string; oldName: string; newName: string }) =>
        invoke_command("qmd_rename_collection", payload),

      qmd_add_context: (payload: { index: string; collection: string; path: string; text: string }) =>
        invoke_command("qmd_add_context", payload),

      qmd_remove_context: (payload: { index: string; collection: string; path: string }) =>
      ...
      qmd_toggle_files: (payload: { index: string; collection: string; repoRoot: string; adds: string[]; removes: string[] }) =>
        invoke_command("qmd_toggle_files", payload),

      qmd_search: (payload: { index: string; query: string; collections?: string[] | null; limit?: number | null }) =>
    ...
    provider_limits: {
      get_provider_limits: () =>
        invoke_command("get_provider_limits", {}),
  ...
  dialogs: {
    pick_directory: (options: { title: string }): Promise<string | null> => {
      return ipcRenderer.invoke("ariadne:dialog", { type: "pick_directory", options });
    },
  },
