...
function start_vite(): ChildProcess {
  ...
  const vite_process = spawn("bunx", ["vite", "--port", "1420", "--strictPort"], {
    cwd: root,
...
async function wait_for_vite(url: string, timeout_ms: number = 30_000): Promise<void> {
  ...
  throw new Error(`Vite dev server did not become ready within ${timeout_ms}ms`);
}
      ...
      setup(build) {
        ...
        build.onEnd((result) => {
          if (result.errors.length > 0) {
            log_error(target.name, `build failed with ${result.errors.length} error(s)`);
            return;
          }
...
async function main(): Promise<void> {
  ...
  esbuild_contexts = await start_esbuild_watchers((target_name) => {
    ...
    if (target_name === "backend") {
      log("backend", "rebuilt — backend-supervisor will pick up changes on next restart");
    }
  });
