...
export async function qmd_rename_index(
  ...
  const raw = await commands.qmd.qmd_rename_index({ oldName: old_name, newName: new_name });
  return qmd_command_result_schema.parse(raw);
}
...
export async function qmd_check_availability(): Promise<QmdAvailability> {
  const raw = await commands.qmd.qmd_check_availability();
...
export async function qmd_get_collection_detail(
  ...
  const raw = await commands.qmd.qmd_get_collection_detail({ index, name });
...
export async function qmd_add_collection(
  ...
  const raw = await commands.qmd.qmd_add_collection({ index, name, path, pattern: pattern ?? null });
  return qmd_command_result_schema.parse(raw);
}
...
export async function qmd_remove_collection(
  ...
  const raw = await commands.qmd.qmd_remove_collection({ index, name });
  return qmd_command_result_schema.parse(raw);
}
...
export async function qmd_rename_collection(
  ...
  const raw = await commands.qmd.qmd_rename_collection({ index, oldName: old_name, newName: new_name });
  return qmd_command_result_schema.parse(raw);
}
...
export async function qmd_add_context(
  ...
  const raw = await commands.qmd.qmd_add_context({ index, collection, path, text });
  return qmd_command_result_schema.parse(raw);
}
...
export async function qmd_remove_context(
  ...
  const raw = await commands.qmd.qmd_remove_context({ index, collection, path });
  return qmd_command_result_schema.parse(raw);
}
...
export async function qmd_scan_filesystem(
  ...
  const raw = await commands.qmd.qmd_scan_filesystem({ index, collection });
...
export async function qmd_get_indexed_paths(
  ...
  const raw = await commands.qmd.qmd_get_indexed_paths({ index, collection });
  return z.array(z.string()).parse(raw);
}
...
export async function qmd_toggle_files(
  ...
  const raw = await commands.qmd.qmd_toggle_files({ index, collection, repoRoot: repo_root, adds, removes });
...
export async function qmd_search(
  ...
  const raw = await commands.qmd.qmd_search({
    index,
    query,
