...
export function CollectionFileTree({
  ...
  const indexed_count = useMemo(() => {
    ...
    return count;
  }, [filesystem_paths, toggle, toggle.pending_adds.size, toggle.pending_removes.size]);
  ...
  const handle_toggle_inclusion = useCallback((node: FileTreeNode) => {
    ...
    return (
      <Card>
        <CardContent className="py-8 text-center text-muted-foreground">
          No files found. The collection path may not exist or the glob pattern matches no files.
        </CardContent>
      </Card>
    );
  }
      ...
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          <CardTitle className="text-base">
            ...
            <span className="text-muted-foreground">/{filesystem_paths.length} indexed</span>
          </CardTitle>
          <div className="flex items-center gap-3">
            {pending_count > 0 && (
              <>
                <span className="text-sm text-yellow-500">{pending_count} pending</span>
  ...
  if (node.is_dir) {
    ...
    if (descendant_paths.length === 0) {
      ...
      for (const p of descendant_paths) {
        ...
        if (toggle.pending_adds.has(p) || toggle.pending_removes.has(p)) has_pending = true;
      }
