...
export function create_qmd_collection_columns(
	index: string,
): ColumnDef<QmdCollection>[] {
	return [
		{
			accessorKey: "name",
			header: ({ column }) => (
		...
		},
		{
			...
			size: 70,
			cell: ({ row }) => (
				<span>{format_number(row.original.active_doc_count)}</span>
			),
		},
		{
			accessorKey: "embedded_count",
			header: ({ column }) => (
				<DataTableColumnHeader column={column} title="Embedded" />
			),
			size: 90,
			cell: ({ row }) => (
				<span>{format_number(row.original.embedded_count)}</span>
			),
		},
		{
			...
			cell: ({ row }) => (
				<span className="text-muted-foreground truncate block">
					{row.original.last_modified
						? format_date_relative(row.original.last_modified)
						: "—"}
				</span>
			),
		},
