...
export function DailyTrend({ data, range_days }: DailyTrendProps) {
	...
	const total_sessions =
		data?.daily_sessions.reduce(
			(s: number, d: { count: number }) => s + d.count,
			0,
		) ?? 0;
	const total_cost =
		data?.daily_cost.reduce(
			(s: number, d: { cost: number }) => s + d.cost,
			0,
		) ?? 0;
			...
			<CardHeader className="pb-2">
				...
				{data && (
					<p className="text-xs text-muted-foreground mt-1">
						...
						{format_cost(total_cost)} · avg {format_cost(avg_cost_per_session)}
						/session
					</p>
				)}
			</CardHeader>
								...
								content={
									<ChartTooltipContent
										labelFormatter={(value) =>
											format_trend_label(value, is_today)
										}
									/>
								}
							/>
