...
export function DataTable<TData, TValue>({
	...
	const [internal_visibility, set_internal_visibility] =
			...
			{toolbar
				...
				: filter_column && (
						<Input
							...
							onChange={(e) =>
								table.getColumn(filter_column)?.setFilterValue(e.target.value)
							}
							className="max-w-sm"
						/>
					)}
						...
						{table.getRowModel().rows.length ? (
							table.getRowModel().rows.map((row) => (
								<TableRow
									...
									onClick={
										on_row_click ? () => on_row_click(row.original) : undefined
									}
								>
