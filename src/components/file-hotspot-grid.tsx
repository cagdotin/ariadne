...
function build_grid_data(
	...
	for (const [full_path, counts] of file_map) {
		...
		const short_name =
			last_slash >= 0 ? relative.slice(last_slash + 1) : relative;
...
export function FileHotspotGrid({
	...
	const page_rows = sorted.slice(
		clamped_page * PAGE_SIZE,
		(clamped_page + 1) * PAGE_SIZE,
	);

	// Directory summaries for current page (to render group headers)
	const dir_summaries = useMemo(
		() => compute_dir_summaries(all_rows),
		[all_rows],
	);
				...
				<div className="flex items-center justify-between gap-3">
					<div>
						...
						<p className="text-xs text-muted-foreground mt-0.5">
							{format_number(all_rows.length)} files ·{" "}
							{format_number(dir_summaries.size)} directories · Cell intensity
							shows relative frequency per column.{" "}
								...
								{show_dir && dir_info && (
									<div className="pt-2 pb-1 px-1 flex items-center justify-between gap-2">
										...
										<span className="text-[10px] text-muted-foreground/50 tabular-nums shrink-0">
											{dir_info.file_count} files · R:{dir_info.reads} E:
											{dir_info.edits} W:{dir_info.writes}
										</span>
									</div>
								)}
								<div
				...
				{total_pages > 1 && (
					<div className="flex items-center justify-between pt-3 mt-2 border-t border-border">
						<span className="text-xs text-muted-foreground">
							{format_number(clamped_page * PAGE_SIZE + 1)}–
							{format_number(
								Math.min((clamped_page + 1) * PAGE_SIZE, sorted.length),
							)}{" "}
							of {format_number(sorted.length)}
						</span>
