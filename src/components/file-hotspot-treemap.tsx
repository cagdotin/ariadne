			...
			{show_label && (
				<>
					<text
						x={x + 5}
						y={scaled_y + 15}
						fontSize={is_dir ? 12 : 10}
						fontWeight={is_dir ? 700 : 400}
						fill="white"
						style={{
		...
		<div className="rounded-none border bg-popover px-3 py-2 text-popover-foreground shadow-md text-xs space-y-1.5 max-w-xs">
			...
			<div className="border-t border-border pt-1 flex items-center justify-between gap-4">
				...
				<span className="tabular-nums font-medium">
					{format_number(total)}
					{is_dir && file_count ? ` · ${file_count} files` : ""}
				</span>
			</div>

			{is_dir && (
				<p className="text-muted-foreground/70 italic pt-0.5">
					Click to explore
				</p>
			)}
		</div>
...
function TreemapLegend({ lens }: { lens: OperationLens }) {
	...
	const color_label =
		lens === "all" ? "dominant operation type" : `${lens} intensity`;
	...
	return (
		<div className="flex flex-wrap items-center gap-x-5 gap-y-1 text-[11px] text-muted-foreground">
			...
			<span>
				<span className="font-medium text-foreground/80">Darker</span> = more
				activity
			</span>
			{lens === "all" && (
				<span className="flex items-center gap-1.5">
					...
					<span
						className="inline-block size-2.5 rounded-sm"
						style={{ background: `hsl(145, 70%, 42%)` }}
					/>
					<span>Edit</span>
					<span
						className="inline-block size-2.5 rounded-sm"
						style={{ background: `hsl(30, 70%, 48%)` }}
					/>
					<span>Write</span>
				</span>
			)}
			{lens !== "all" && (
				<span className="flex items-center gap-1.5">
					<span
						className="inline-block size-2.5 rounded-sm"
						style={{ background: `hsl(${OP_HUE[lens]}, 70%, 45%)` }}
					/>
			...
			<CardHeader className="space-y-2">
				<div className="flex items-baseline justify-between gap-3">
					...
					<span className="text-xs text-muted-foreground">
						Sized by{" "}
						<span className="font-medium text-foreground/80">{lens_name}</span>
					</span>
				</div>
				...
				<p className="text-xs text-muted-foreground leading-relaxed">
					Click a directory to explore its files. Use the breadcrumb bar below
					the chart to navigate back.
				</p>
			</CardHeader>
