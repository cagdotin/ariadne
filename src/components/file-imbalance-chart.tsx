...
import { intensity_fill, OP_HUE } from "@/lib/file-analytics";
...
export function FileImbalanceChart({
	insights,
	project_path,
}: FileImbalanceChartProps) {
		...
		<Card className="min-w-0 overflow-hidden">
			<CardHeader className="space-y-1">
				...
				<p className="text-xs text-muted-foreground leading-relaxed">
					Files ranked by how skewed their operation mix is. Bars show the{" "}
					<span className="text-blue-400 font-medium">read</span> vs{" "}
					<span className="text-green-400 font-medium">edit</span>+
					<span className="text-orange-400 font-medium">write</span> split.
					Files near the extremes are unusual — either read-only references or
					heavily mutated.
				</p>
			</CardHeader>
			<CardContent ref={ref}>
				<div className="space-y-px">
					{rows.map((row) => {
						const read_w =
							bar_width > 0 ? Math.round(row.read_ratio * bar_width) : 0;
						const change_w = bar_width > 0 ? bar_width - read_w : 0;

						// Intensity: brighter if the dominant side has more ops
						const read_intensity =
							row.read_ratio > 0 ? 0.4 + row.read_ratio * 0.6 : 0;
						const change_intensity =
							row.change_ratio > 0 ? 0.4 + row.change_ratio * 0.6 : 0;
						...
						const change_hue =
							edit_ratio >= write_ratio ? OP_HUE.edit : OP_HUE.write;
