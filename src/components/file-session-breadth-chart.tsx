...
export function FileSessionBreadthChart({
	insights,
	lens,
	project_path,
}: FileSessionBreadthChartProps) {
	...
	const has_breadth_data = useMemo(
		() =>
			insights.some(
				(f) => f.distinct_session_count != null && f.distinct_session_count > 0,
			),
		[insights],
	);
	...
	const rows = useMemo(() => {
		...
		const eligible = insights.filter(
			(f) =>
				f.distinct_session_count != null &&
				f.distinct_session_count > 0 &&
				get_lens_value(f, lens) > 0,
		);
		...
		return (
			<Card className="min-w-0 overflow-hidden">
				...
				<CardContent>
					<div className="flex h-32 items-center justify-center rounded-none border border-dashed">
						<p className="text-sm text-muted-foreground">
							No files with {lens === "all" ? "" : `${lens} `}activity in the
							current scope.
						</p>
					</div>
				</CardContent>
		...
		<Card className="min-w-0 overflow-hidden">
			<CardHeader className="space-y-1">
				...
				<p className="text-xs text-muted-foreground leading-relaxed">
					Files ranked by how many distinct sessions touched them. Bar width =
					sessions, color intensity = {lens_label} per session. Broadly
					important files appear at the top; one-session noise sinks to the
					bottom.
				</p>
			</CardHeader>
			<CardContent ref={ref}>
					...
					{rows.map((row) => {
						const bar_w =
							bar_area > 0
								? Math.max(
										4,
										Math.round((row.session_count / max_sessions) * bar_area),
									)
								: 4;
						const per_session = row.ops_per_session;
						const max_per_session =
							max_ops / Math.max(1, Math.sqrt(max_sessions));
						const bucket = intensity_bucket(
							per_session,
							max_per_session > 0 ? max_per_session : 1,
						);
						const hue =
							lens === "all"
