	...
	return (
		<div className="rounded-none border bg-popover px-3 py-2 text-popover-foreground shadow-md text-xs space-y-1 max-w-xs">
			...
			<div className="flex gap-3">
				<span
					className={`text-blue-400 ${current_lens === "read" ? "font-bold" : ""}`}
				>
					R: {format_number(d.reads)}
				</span>
				<span
					className={`text-green-400 ${current_lens === "edit" ? "font-bold" : ""}`}
				>
					E: {format_number(d.edits)}
				</span>
				<span
					className={`text-orange-400 ${current_lens === "write" ? "font-bold" : ""}`}
				>
					W: {format_number(d.writes)}
				</span>
			</div>
			<p className="text-muted-foreground">
				{format_number(d.total)} total ops
			</p>
		</div>
	);
}
...
export function FileSizeActivityScatter({
	insights,
	lens,
	project_path,
}: FileSizeActivityScatterProps) {
	...
	const fetch_sizes = useCallback(async () => {
		...
		try {
			...
			const missing = result.filter(
				(r) => r.file_size_bytes == null || r.file_size_bytes === undefined,
			).length;
	...
	}, [insights.map, insights.length, insights]); // eslint-disable-line react-hooks/exhaustive-deps
		...
		return enriched
			...
			.map((f) => {
				...
				return {
					...
					full_path: f.path,
					// biome-ignore lint/style/noNonNullAssertion: filtered to only entries with file_size_bytes
		...
		<Card className="min-w-0 overflow-hidden">
			<CardHeader className="space-y-1">
				...
				<p className="text-xs text-muted-foreground leading-relaxed">
					Each dot is a file. X = file size, Y = {lens_label}. Small-but-hot
					files appear in the top-left. Large quiet files appear in the
					bottom-right.
					{lens === "all" && (
						<span>
							{" "}
			...
			<CardContent ref={ref}>
				...
				{load_state === "loaded" && points.length === 0 && (
					<div className="flex h-32 items-center justify-center rounded-none border border-dashed">
						<p className="text-sm text-muted-foreground">
							No files with both known size and{" "}
							{lens === "all" ? "" : `${lens} `}activity.
						</p>
					</div>
				)}
					...
					<>
						...
						{skipped_count > 0 && (
							<p className="text-[11px] text-muted-foreground/70 mt-1">
								{skipped_count} file{skipped_count !== 1 ? "s" : ""} excluded —
								size unavailable (deleted or inaccessible).
							</p>
						)}
					</>
				)}
