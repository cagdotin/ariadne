...
export function IndexSelector({
	...
	const commit_rename = () => {
		...
		if (
			indexes.some((idx) => idx.name === trimmed && idx.name !== renaming_index)
		) {
						...
						{idx.name !== "default" && !is_renaming && (
							<DropdownMenu>
								...
								<DropdownMenuContent align="end" side="bottom" sideOffset={4}>
									<DropdownMenuItem onClick={() => start_rename(idx.name)}>
			...
			<div className="ml-auto shrink-0 self-center flex items-center gap-0.5">
				...
				<InfoTip title="QMD Indexes" side="bottom" align="end">
					<div className="space-y-2">
						<p>
							Each index is an independent knowledge base with its own
							collections, documents, and vector embeddings.
						</p>
						<p>
							Use separate indexes to organize different domains — personal
							notes, work docs, project references — without mixing them
							together.
						</p>
					</div>
				</InfoTip>
