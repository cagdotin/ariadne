...
const group_by_weeks = (
	...
	const padded_activities = [
		...(new Array(
			differenceInCalendarDays(first_date, first_calendar_date),
...
const get_month_labels = (
	...
	return weeks
		.reduce<MonthLabel[]>((labels, week, week_index) => {
			...
			if (!month) {
				const month_name = new Date(first_activity.date).toLocaleString(
					"en-US",
					{
						month: "short",
					},
...
export const ContributionGraph = ({
	...
	const weeks = useMemo(
		() => group_by_weeks(data, week_start),
		[data, week_start],
	);
...
export const ContributionGraphCalendar = ({
	...
	return (
		<div
			className={cn(
				responsive ? "w-full" : "max-w-full overflow-x-auto overflow-y-hidden",
				className,
			)}
			{...props}
		>
