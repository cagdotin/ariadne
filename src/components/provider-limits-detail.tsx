import { RefreshCw, Clock, AlertCircle, Gauge, Info } from "lucide-react";
      ...
      <div className="flex items-start justify-between">
        <div className="space-y-1">
          ...
          {snapshot.account_label && (
            <p className="text-xs text-muted-foreground">{snapshot.account_label}</p>
          )}
        </div>
        <div className="text-right space-y-0.5">
          ...
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger render={<span />} className="text-[10px] text-muted-foreground flex items-center gap-1 justify-end cursor-help">
                <Info className="size-2.5" />
                {source_label(snapshot.source).split("(")[0].trim()}
              </TooltipTrigger>
              <TooltipContent>
                <p>{source_label(snapshot.source)}</p>
                <p className="text-muted-foreground">{confidence_label(snapshot.source_confidence)}</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        </div>
      ...
      {snapshot.credits && (
        <div className="text-xs text-muted-foreground border-t border-border pt-2">
          {snapshot.credits.unlimited ? (
            ...
            <span>Credits balance: {snapshot.credits.balance ?? "available"}</span>
          ) : (
