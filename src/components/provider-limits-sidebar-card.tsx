...
import { Gauge, RefreshCw, AlertCircle, Clock } from "lucide-react";
  ...
  return (
    <div className="space-y-1">
      <div className="flex items-center justify-between text-[10px]">
        ...
        <span className="text-foreground tabular-nums font-medium">{used.toFixed(0)}%</span>
      </div>
      <Progress
        value={used}
...
function CollapsedIndicator({ snapshots }: { snapshots: ProviderLimitSnapshot[] }) {
  ...
  const summary = useMemo(() => {
    ...
    if (primary_used != null) return `Primary: ${primary_used.toFixed(0)}% used`;
