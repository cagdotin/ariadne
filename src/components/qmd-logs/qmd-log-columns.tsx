  ...
  return [
    {
      ...
      cell: ({ row }) => (
        <span className="text-muted-foreground text-xs whitespace-nowrap">
          {row.original.timestamp ? format_date_relative(row.original.timestamp) : "—"}
        </span>
      ),
    },
    {
      ...
      size: 130,
      cell: ({ row }) => (
        <span className="text-sm truncate block" title={row.original.project_path}>
    ...
    },
    {
      ...
      header: ({ column }) => <DataTableColumnHeader column={column} title="Subcommand" />,
      size: 100,
      cell: ({ row }) => (
    ...
    },
    {
      ...
      header: ({ column }) => <DataTableColumnHeader column={column} title="Argument" />,
      cell: ({ row }) => (
        <span className="text-sm truncate block" title={row.original.primary_argument ?? undefined}>
    ...
    },
    {
      ...
      cell: ({ row }) => {
        if (!row.original.has_output) {
          return <Badge variant="outline" className="text-[11px]">incomplete</Badge>;
        }
        return row.original.is_error ? (
          <Badge variant="destructive" className="text-[11px]">error</Badge>
        ) : (
          <Badge variant="secondary" className="text-[11px] text-emerald-600 dark:text-emerald-400">ok</Badge>
        );
      },
    },
    {
