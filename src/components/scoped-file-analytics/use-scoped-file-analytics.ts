...
export function use_scoped_file_analytics(
	...
	const [file_stats, set_file_stats] =
		useState<UseScopedFileAnalyticsState["file_stats"]>(null);
	const [loading, set_loading] = useState(true);
	const [error, set_error] = useState<string | null>(null);
	const [exclude_paths, set_exclude_paths] = useState("");
	const [active_tab, set_active_tab] =
		useState<UseScopedFileAnalyticsState["active_tab"]>("read");
	...
	const excludes = useMemo(
		() => parse_excludes(exclude_paths),
		[exclude_paths],
	);

	const filtered_read = useMemo(
		() =>
			(file_stats?.read_files ?? []).filter(
				(file) => !is_excluded(file.name, excludes),
			),
		[file_stats, excludes],
	);
