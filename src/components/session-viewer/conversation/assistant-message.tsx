...
export function AssistantMessage({
	...
	const text_blocks = msg.content.filter(
		(b) => b.type === "text" && "text" in b && (b.text as string).trim(),
	);
	const thinking_blocks = msg.content.filter(
		(b) =>
			b.type === "thinking" && "thinking" in b && (b.thinking as string).trim(),
	);
	...
	const resolved_tools: ResolvedToolCall[] = tool_calls.map((tc) => ({
		...
		arguments:
			"arguments" in tc ? (tc.arguments as Record<string, unknown>) : {},
		...
		<div className="relative space-y-2 min-w-0 overflow-hidden">
			...
			{thinking_blocks.map((block, i) => (
				<ThinkingBlock
					...
					key={i}
					text={"thinking" in block ? (block.thinking as string) : ""}
				/>
			))}
				...
				<div className="px-3">
					{text_blocks.map((block, i) => (
						<MarkdownContent
							// biome-ignore lint/suspicious/noArrayIndexKey: static list, index is stable
							key={i}
							content={"text" in block ? (block.text as string) : ""}
						/>
					))}
				</div>
			)}
