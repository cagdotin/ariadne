...
export function SessionSidebarAnalytics({
	session,
}: SessionSidebarAnalyticsProps) {
	const tools = useMemo(
		() => to_tool_aggregates(session.tool_calls),
		[session.tool_calls],
	);
	const bash_commands = useMemo(
		() => to_name_counts(session.bash_commands),
		[session.bash_commands],
	);
	const read_files = useMemo(
		() => to_name_counts(session.read_files),
		[session.read_files],
	);
	const edit_files = useMemo(
		() => to_name_counts(session.edit_files),
		[session.edit_files],
	);
	const write_files = useMemo(
		() => to_name_counts(session.write_files),
		[session.write_files],
	);
				...
				{models.length === 0 ? (
					...
					<div className="space-y-1.5">
						{models.map((m) => (
							...
							>
								<span
									className="text-[11px] font-mono text-foreground truncate min-w-0"
									title={`${m.provider}/${m.model_id}`}
								>
									{m.model_id}
								</span>
								<Badge
