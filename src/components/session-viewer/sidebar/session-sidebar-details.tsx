...
import type { SessionEntry, SessionHeader } from "../types";
import { compute_stats } from "../utils";
import { BreakdownRow, MetricCell, SectionLabel } from "./sidebar-primitives";
...
export function SessionSidebarDetails({
	header,
	entries,
}: SessionSidebarDetailsProps) {
	const stats = compute_stats(entries);
	const total_cost =
		stats.cost.input +
		stats.cost.output +
		stats.cost.cache_read +
		stats.cost.cache_write;
	const total_tokens =
		stats.tokens.input +
		stats.tokens.output +
		...
		<div className="flex flex-col h-full overflow-y-auto">
			...
			<div className="px-5 pt-5 pb-4 border-b border-border/50">
				...
				<div className="text-[10px] text-muted-foreground font-mono leading-relaxed space-y-1">
					<div className="truncate" title={header?.id}>
						{header?.id ?? "—"}
					</div>
					<div>{start_date}</div>
					{cwd && (
						<div className="truncate opacity-60" title={cwd}>
							{cwd}
						</div>
					)}
				</div>
			...
			<div className="px-5 py-4 border-b border-border/50">
				...
				<div className="mt-3 space-y-2.5">
					<BreakdownRow
						...
						bar_pct={
							total_tokens > 0 ? (stats.tokens.input / total_tokens) * 100 : 0
						}
						color="var(--chart-1)"
					/>
					<BreakdownRow
						...
						bar_pct={
							total_tokens > 0 ? (stats.tokens.output / total_tokens) * 100 : 0
						}
						color="var(--chart-3)"
					/>
					<BreakdownRow
						...
						bar_pct={
							total_tokens > 0
								? (stats.tokens.cache_read / total_tokens) * 100
								: 0
						}
						color="var(--chart-5)"
					/>
					<BreakdownRow
						...
						bar_pct={
							total_tokens > 0
								? (stats.tokens.cache_write / total_tokens) * 100
								: 0
						}
						color="var(--chart-4)"
					/>
			...
			<div className="px-5 py-4 border-b border-border/50">
				...
				<div className="mt-3 space-y-2.5">
					...
					<BreakdownRow
						...
						bar_pct={
							total_cost > 0 ? (stats.cost.output / total_cost) * 100 : 0
						}
						color="var(--chart-3)"
					/>
					<BreakdownRow
						...
						bar_pct={
							total_cost > 0 ? (stats.cost.cache_read / total_cost) * 100 : 0
						}
						color="var(--chart-5)"
					/>
					<BreakdownRow
						...
						bar_pct={
							total_cost > 0 ? (stats.cost.cache_write / total_cost) * 100 : 0
						}
						color="var(--chart-4)"
					/>
				</div>
