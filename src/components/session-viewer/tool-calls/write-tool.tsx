...
import { shorten_path, get_language_from_path } from "../utils";
    ...
    return {
      ...
      summary: (
        <>
          ...
          {lines > 0 && (
            <span className="text-muted-foreground/50 ml-1">({lines} lines)</span>
          )}
        </>
      ),
    };
  ...
  get_body(tool, output) {
    ...
    return (
      <>
        ...
        {output && (
          <div className="mt-1 text-xs text-muted-foreground font-mono">{output}</div>
        )}
      </>
    );
  },
