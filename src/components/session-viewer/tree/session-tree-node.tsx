...
interface SessionTreeNodeProps {
  ...
  is_active: boolean;
  is_on_path: boolean;
  tool_call_map: Map<string, { name: string; arguments: Record<string, unknown> }>;
  on_click: () => void;
}
  ...
  switch (entry.type) {
    case "message": {
      ...
      if (msg.role === "user") {
        ...
        return <><span className="text-primary">user:</span> {text}</>;
      }
      if (msg.role === "assistant") {
        const text = truncate(normalize(extract_text(msg.content)));
        if (text) return <><span className="text-chart-1">asst:</span> {text}</>;
        if ("stopReason" in msg && msg.stopReason === "aborted")
          return <><span className="text-chart-1">asst:</span> <span className="text-muted-foreground">(aborted)</span></>;
        if ("errorMessage" in msg && msg.errorMessage)
          return <><span className="text-chart-1">asst:</span> <span className="text-destructive">{truncate(String(msg.errorMessage))}</span></>;
        return <><span className="text-chart-1">asst:</span> <span className="text-muted-foreground">(no text)</span></>;
      }
      if (msg.role === "toolResult") {
        const tc = msg.toolCallId ? tool_call_map.get(msg.toolCallId) : null;
        if (tc) return <span className="text-muted-foreground">{format_tool_call_label(tc.name, tc.arguments)}</span>;
        return <span className="text-muted-foreground">[{msg.toolName ?? "tool"}]</span>;
      }
      if (msg.role === "bashExecution") {
        const cmd = truncate(normalize(msg.command || ""));
        return <><span className="text-muted-foreground">[bash]:</span> {cmd}</>;
      }
      return <span className="text-muted-foreground">[{msg.role}]</span>;
    }
    case "compaction":
      return <span className="text-chart-4">[compaction: {Math.round((entry as { tokensBefore: number }).tokensBefore / 1000)}k tokens]</span>;
    case "branch_summary":
      return <><span className="text-chart-2">[branch summary]:</span> {truncate(normalize((entry as { summary: string }).summary || ""))}</>;
    case "custom_message":
      return <><span className="text-chart-5">[{(entry as { customType: string }).customType}]</span></>;
    case "model_change":
      return <span className="text-muted-foreground">[model: {(entry as { modelId: string }).modelId}]</span>;
    case "thinking_level_change":
      return <span className="text-muted-foreground">[thinking: {(entry as { thinkingLevel: string }).thinkingLevel}]</span>;
    default:
