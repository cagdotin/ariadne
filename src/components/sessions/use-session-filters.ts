...
export function use_session_filters(sessions: SessionSummary[]) {
	...
	const [selected_models, set_selected_models] = useState<Set<string>>(
		new Set(),
	);
	...
	const has_active_filters =
		search.length > 0 || selected_tools.size > 0 || selected_models.size > 0;
	...
	const filtered_sessions = useMemo(() => {
		...
		if (search.length > 0) {
			...
			result = result.filter((s) => {
				const haystack = [s.id, s.title, s.project_name, s.first_user_message]
					.filter(Boolean)
					.join(" ")
					.toLowerCase();
