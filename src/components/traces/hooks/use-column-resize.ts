...
export function use_column_resize(container_ref: React.RefObject<HTMLDivElement | null>) {
  const [tree_width_pct, set_tree_width_pct] = useState(DEFAULT_TREE_WIDTH_PCT);
  const resize_ref = useRef<{ start_x: number; start_pct: number } | null>(null);
  ...
  const handle_divider_mouse_down = useCallback((e: React.MouseEvent) => {
    e.preventDefault();
    const container = container_ref.current;
    if (!container) return;

    resize_ref.current = { start_x: e.clientX, start_pct: tree_width_pct_ref.current };
    ...
    const handle_mouse_move = (move_e: MouseEvent) => {
      ...
      const new_pct = Math.min(MAX_TREE_WIDTH_PCT, Math.max(MIN_TREE_WIDTH_PCT, resize_ref.current.start_pct + delta_pct));
