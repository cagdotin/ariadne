...
export function use_timeline_zoom({
	total_duration_ms,
	timeline_ref,
	container_ref,
	...
	}, [container_ref.current]); // stable — reads from refs

	// Drag-to-zoom on timeline ruler area
	const handle_zoom_mouse_down = useCallback(
		(e: React.MouseEvent) => {
			...
			const handle_mouse_up = (up_e: MouseEvent) => {
				...
				const max_px = Math.min(
					timeline_left + timeline_width,
					Math.max(start_px, end_px),
				);
				...
				if (max_pct - min_pct > 2) {
					...
					const new_start =
						current_view_start + (min_pct / 100 / SCALE) * range;
	...
	const zoom_overlay_left = zoom_drag
		? Math.min(zoom_drag.start_px, zoom_drag.current_px)
		: 0;
	const zoom_overlay_width = zoom_drag
		? Math.abs(zoom_drag.current_px - zoom_drag.start_px)
		: 0;
