...
export const SpanBar = memo(function SpanBar({ node, total_duration_ms, is_collapsed_parent, is_selected, view_start_ms, view_end_ms, on_inspect }: SpanBarProps) {
  ...
  const left_pct = range > 0 ? ((bar_start - v_start) / range) * 100 * SCALE : 0;
  const raw_width_pct = range > 0 ? Math.max(((bar_end - bar_start) / range) * 100 * SCALE, 0.3) : 0.3;
