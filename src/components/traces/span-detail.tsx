...
function FieldRow({ label, value }: { label: string; value: React.ReactNode }) {
	return (
		<div className="flex items-start justify-between gap-2 px-3 py-0.5 text-xs">
			...
			<span className="text-right font-mono text-foreground break-all">
				{value}
			</span>
		</div>
	);
...
function extract_content_preview(node: SpanNode): React.ReactNode {
	...
	if (message.role === "assistant") {
		...
		if (assistant.content && Array.isArray(assistant.content)) {
			const text_block = assistant.content.find(
			...
			if (assistant.usage) {
				...
				usage_node = (
					<>
						...
						<div className="flex flex-col gap-0.5">
							{u.input !== undefined && (
								<FieldRow label="Input" value={u.input.toLocaleString()} />
							)}
							{u.output !== undefined && (
								<FieldRow label="Output" value={u.output.toLocaleString()} />
							)}
							{u.cacheRead !== undefined && (
								<FieldRow
									label="Cache read"
									value={u.cacheRead.toLocaleString()}
								/>
							)}
							{u.cost?.total !== undefined && (
								<FieldRow label="Cost" value={`$${u.cost.total.toFixed(4)}`} />
							)}
						</div>
					</>
				);
	...
	return (
		<div className="overflow-y-auto h-full">
			...
			<div className="space-y-0.5 py-2">
				...
				<FieldRow
					label="Duration"
					value={`${Math.round(node.duration_ms)}ms`}
				/>
