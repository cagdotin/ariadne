...
export const SpanTreeCell = memo(function SpanTreeCell({
	row,
	is_selected,
	on_toggle,
}: SpanTreeCellProps) {
	...
	const display_duration = is_collapsed_parent
		? node.subtree_duration_ms
		: node.duration_ms;
	...
	return (
		...
		>
			...
			{/* Label */}
			<span className="ml-1 truncate text-[11px]">{node.label}</span>
