...
export function TimelineRuler({ total_duration_ms, view_start_ms, view_end_ms }: TimelineRulerProps) {
  ...
  return (
    <div className="relative h-6 bg-muted/30">
      {ticks.map((tick_ms) => {
        const left_pct = range > 0 ? ((tick_ms - v_start) / range) * 100 * SCALE : 0;
        ...
        return (
          ...
          >
            <span
              ...
              style={{ transform: is_first ? "translateX(0)" : "translateX(-50%)" }}
            >
