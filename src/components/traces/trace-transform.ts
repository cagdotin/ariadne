...
import type { SpanKind, SpanNode, SpanTree } from "./types";
...
function build_assistant_children(
	...
	if (content.length === 1 && (content[0] as ContentBlock).type === "text")
		return [];
...
function find_tool_call_child(
	assistant_node: SpanNode,
	tool_call_id: string,
): SpanNode | null {
	for (const child of assistant_node.children) {
		if (child.kind !== "tool_call" || child.content_index === undefined)
			continue;
	...
	for (let i = 0; i < entries.length; i++) {
		...
		if (entry.type !== "message") {
			...
			const label_map: Record<string, string> = {
				...
				custom_message:
					(entry as { customType?: string }).customType ?? "CUSTOM",
			};
		...
		switch (message.role) {
			...
			case "assistant": {
				...
				const label = assistant_msg.errorMessage
					? "Assistant (error)"
					: "Assistant";
				const node: SpanNode = {
					id: make_id(entry.id),
					...
					depth: 0,
					children: build_assistant_children(
						entry,
						assistant_msg,
						start_ms,
						end_ms,
			...
			case "toolResult": {
				...
				if (current_assistant && result_msg.toolCallId) {
					const tool_call_child = find_tool_call_child(
						current_assistant,
						result_msg.toolCallId,
					);
					if (tool_call_child) {
						// Extend tool call span to cover the result — absorb result into the tool call
						tool_call_child.end_ms = Math.max(
							end_ms,
							tool_call_child.start_ms + MIN_SPAN_MS,
						);
						tool_call_child.duration_ms =
							tool_call_child.end_ms - tool_call_child.start_ms;
			...
			case "bashExecution": {
				...
				const is_error =
					bash_msg.exitCode !== 0 && bash_msg.exitCode !== undefined;
