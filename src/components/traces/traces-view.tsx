...
import { build_span_tree, flatten_span_tree, format_duration } from "./trace-transform";
...
export function TracesView({ header, entries }: TracesViewProps) {
  const tree = useMemo(() => build_span_tree(header, entries), [header, entries]);
  ...
  // Expansion state
  const { expanded, is_all_expanded, toggle, toggle_all } = use_span_tree_expansion(tree.roots);

  // Flatten visible rows
  const visible_rows = useMemo(() => flatten_span_tree(tree.roots, expanded), [tree.roots, expanded]);

  // Selection + inspector panel
  const { panel, set_panel } = use_session_panel();
  const { selected_id, selected_node, show_inspector, select, deselect } = use_span_selection({
    roots: tree.roots,
    panel,
    set_panel,
  });
  ...
  // Column resize
  const { tree_width_pct, handle_divider_mouse_down } = use_column_resize(container_ref);

  if (entries.length === 0) {
    return (
      <div className="flex items-center justify-center h-full p-8">
        <p className="text-muted-foreground text-sm">No entries in this session</p>
      </div>
    );
  }
  ...
  const timeline_content = (
    <div className="flex flex-col h-full min-h-0 overflow-hidden">
      ...
      <div className="flex items-center gap-3 px-3 py-1.5 border-b border-border bg-muted/30 flex-none">
        <Badge variant="outline" className="text-[10px]">{stats.event_count} events</Badge>
        <Badge variant="outline" className="text-[10px]">{stats.tool_count} tools</Badge>
        {stats.error_count > 0 && (
          <Badge variant="destructive" className="text-[10px]">{stats.error_count} errors</Badge>
        )}
        {stats.model && (
          <span className="text-[10px] text-muted-foreground">{stats.model}</span>
        )}
        ...
        <div className="ml-auto flex items-center gap-1">
          <Button variant="ghost" size="sm" className="h-6 px-2 text-[10px]" onClick={toggle_all}>
      ...
      >
        ...
        <div className="flex flex-none border-b border-border">
          <div className="flex-none overflow-hidden flex items-center px-3 border-r border-border" style={{ width: "var(--tree-width)" }}>
