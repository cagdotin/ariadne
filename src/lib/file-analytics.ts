 ...
 */
export function from_backend_insights(records: FileInsightRecord[]): FileInsight[] {
...
export function enrich_with_sizes(
  ...
  return insights.map((i) => ({
    ...i,
    file_size_bytes: size_map.has(i.path) ? size_map.get(i.path)! : undefined,
  }));
}
...
export function merge_file_insights(
  ...
  const ensure = (path: string): FileInsight => {
    ...
    if (!entry) {
      entry = { path, read_count: 0, edit_count: 0, write_count: 0, total_count: 0 };
      map.set(path, entry);
    }
    return entry;
  };
  ...
  for (const insight of map.values()) {
    insight.total_count = insight.read_count + insight.edit_count + insight.write_count;
  }
...
export function get_lens_value(insight: FileInsight, lens: OperationLens): number {
  switch (lens) {
    case "all": return insight.total_count;
    case "read": return insight.read_count;
    case "edit": return insight.edit_count;
    case "write": return insight.write_count;
  }
}
...
export function dominant_op(r: number, e: number, w: number): "read" | "edit" | "write" {
