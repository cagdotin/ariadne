...
function QmdLogsEnabled() {
	...
	const [subcommand_filter, set_subcommand_filter] = useState<string | null>(
		null,
	);
	const [error_only, set_error_only] = useState(false);

	// Modal state
	const [selected_entry, set_selected_entry] = useState<QmdLogEntry | null>(
		null,
	);
				...
				<div className="rounded-none border border-border p-12 text-center space-y-3">
					...
					{logs.length === 0 && (
						<p className="text-xs text-muted-foreground">
							QMD logs are extracted from agent session files. Once agents use
							the QMD CLI, their calls will appear here.
						</p>
					)}
				</div>
			) : (
