...
import { format_cost, format_number, format_tokens } from "@/lib/format";
...
function TokenBreakdown({ overview }: { overview: AnalyticsOverview }) {
	const categories = [
		{
			key: "input",
			name: "Input",
			tokens: overview.input_tokens,
			color: "var(--chart-1)",
		},
		{
			key: "output",
			name: "Output",
			tokens: overview.output_tokens,
			color: "var(--chart-3)",
		},
		{
			key: "cache_read",
			name: "Cache Read",
			tokens: overview.cache_read_tokens,
			color: "var(--chart-5)",
		},
		{
			key: "cache_write",
			name: "Cache Write",
			tokens: overview.cache_write_tokens,
			color: "var(--chart-4)",
		},
	].filter((c) => c.tokens > 0);
	...
	const cache_hit_rate =
		cache_total > 0 ? (overview.cache_read_tokens / cache_total) * 100 : 0;
	...
	return (
		<Card className="min-w-0 overflow-hidden">
			<CardHeader>
				...
				<p className="text-xs text-muted-foreground">
					{format_cost(time_data.total_cost)} total · avg{" "}
					{format_cost(time_data.avg_cost_per_session)}/session
				</p>
			</CardHeader>
			<CardContent>
...
function CostStatCards({ overview, time_data }: CostTabProps) {
	...
	return (
		<div className="flex flex-wrap gap-3">
			...
			<MiniStat
				label="Total Tokens"
				value={format_tokens(overview.total_tokens)}
			/>
		</div>
	);
}
