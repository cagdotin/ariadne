...
function FileSummaryCards({ stats }: { stats: ProjectFileStats }) {
	return (
		<div className="flex flex-wrap gap-3">
			...
			<MiniStat
				label="Files Read"
				value={format_number(stats.read_files.length)}
			/>
			<MiniStat
				label="Files Edited"
				value={format_number(stats.edit_files.length)}
			/>
			<MiniStat
				label="Files Written"
				value={format_number(stats.write_files.length)}
			/>
		</div>
	);
}
...
function OperationLensPicker({
	...
	return (
		<Tabs
			value={value}
			onValueChange={(v) => on_change(v as OperationLens)}
			className="shrink-0"
		>
			<TabsList>
			...
			<div className="flex min-w-0 flex-wrap items-center gap-3">
				...
				<div className="flex min-w-0 flex-1 items-center gap-3">
					<label
						...
						htmlFor="files-tab-exclude-paths"
					>
						Exclude paths:
					</label>
					<Input
						id="files-tab-exclude-paths"
						value={exclude_paths}
