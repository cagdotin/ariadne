...
function PatternStatCards({ time_data, range_days }: PatternsTabProps) {
  const range_label = range_days === 0 ? "All time" : range_days === 1 ? "Today" : `Last ${range_days}d`;

  // Find the busiest day of week and time of day
  const peak_day = time_data.by_weekday.reduce((a, b) => (b.sessions > a.sessions ? b : a), time_data.by_weekday[0]);
  const peak_time = time_data.by_time_of_day.reduce((a, b) => (b.sessions > a.sessions ? b : a), time_data.by_time_of_day[0]);
  const active_days = time_data.daily_sessions.length;

  return (
    <div className="flex flex-wrap gap-3">
      <MiniStat label="Sessions" value={format_number(time_data.total_sessions)} sub={range_label} />
      <MiniStat label="Active Days" value={format_number(active_days)} sub={range_label} />
      <MiniStat label="Peak Day" value={peak_day?.day ?? "—"} sub={peak_day ? `${peak_day.sessions} sessions` : undefined} />
      <MiniStat label="Peak Time" value={peak_time?.label ?? "—"} sub={peak_time ? `${peak_time.sessions} sessions` : undefined} />
    </div>
  );
}
...
function SessionsTrend({ time_data }: { time_data: TimeBreakdown }) {
  ...
  return (
    <Card className="min-w-0 overflow-hidden">
      ...
      <CardContent>
        <ChartContainer config={sessions_trend_config} className="h-[200px] w-full">
