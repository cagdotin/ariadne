...
vi.mock("../../../backend/qmd/index-paths", () => ({
  resolve_index_db_path: vi.fn((name: string) => `/cache/${name === "default" ? "index" : name}.sqlite`),
  validate_index_name: vi.fn((name: string) => {
    if (!/^[a-z][a-z0-9-]*$/.test(name)) return { valid: false, error: "Invalid name" };
    if (name === "index" || name === "models") return { valid: false, error: `'${name}' is reserved` };
    return { valid: true };
  }),
}));
...
describe("qmd_delete_index", () => {
  it("throws when trying to delete default index", () => {
    expect(() => qmd_delete_index("default")).toThrow("Cannot delete the default index");
  });
  ...
  it("deletes main file and WAL/SHM companions", () => {
    ...
    expect(fs.default.unlinkSync).toHaveBeenCalledWith("/cache/my-index.sqlite");
    expect(fs.default.unlinkSync).toHaveBeenCalledWith("/cache/my-index.sqlite-wal");
    expect(fs.default.unlinkSync).toHaveBeenCalledWith("/cache/my-index.sqlite-shm");
  });
...
describe("qmd_rename_index", () => {
  it("throws when trying to rename default index", () => {
    expect(() => qmd_rename_index("default", "new-name")).toThrow("Cannot rename the default index");
  });
  ...
  it("renames main file successfully", () => {
    ...
    expect(fs.default.renameSync).toHaveBeenCalledWith("/cache/old.sqlite", "/cache/new-name.sqlite");
  });
  ...
  it("renames WAL and SHM files if they exist", () => {
    ...
    expect(fs.default.renameSync).toHaveBeenCalledWith("/cache/old.sqlite-wal", "/cache/new-name.sqlite-wal");
    expect(fs.default.renameSync).toHaveBeenCalledWith("/cache/old.sqlite-shm", "/cache/new-name.sqlite-shm");
  });
});
