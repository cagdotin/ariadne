...
import { get_all_sessions, get_session_detail } from "../../../backend/analytics/query";
...
function make_session(overrides: Partial<SessionSummary> = {}): SessionSummary {
  return {
    id: "s1", project_path: "/proj", project_name: "proj",
    session_dir: "dir", file_name: "s.jsonl", file_size_bytes: 1000,
    started_at: "2025-06-01T12:00:00Z", ended_at: null, duration_seconds: null,
    title: null, first_user_message: null,
    total_cost: 0, input_cost: 0, output_cost: 0, cache_read_cost: 0, cache_write_cost: 0,
    total_tokens: 0, input_tokens: 0, output_tokens: 0, cache_read_tokens: 0, cache_write_tokens: 0,
    user_message_count: 0, assistant_message_count: 0, tool_result_count: 0,
    turn_count: 0, compaction_count: 0,
    tool_calls: {}, bash_commands: {}, read_files: {}, edit_files: {}, write_files: {},
    models_used: [],
    ...overrides,
  };
}
...
describe("get_session_detail", () => {
  ...
  it("throws for non-existent session", async () => {
    ...
    await expect(get_session_detail("missing"))
      .rejects.toThrow("Session with id missing not found");
  });

  it("throws for empty sessions list", async () => {
    await expect(get_session_detail("any"))
      .rejects.toThrow("Session with id any not found");
  });
});
