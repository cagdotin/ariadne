...
function make_session(overrides: Partial<SessionSummary> = {}): SessionSummary {
  return {
    ...
    session_dir: "proj-dir", file_name: "session.jsonl", file_size_bytes: 1000,
...
describe("get_session_entries", () => {
  it("throws when session not found in cache", async () => {
    await expect(get_session_entries("nonexistent"))
  ...
  it("throws when session file does not exist on disk", async () => {
    ...
    await expect(get_session_entries("s1"))
  ...
  it("parses JSONL and extracts header, entries, and leaf_id", async () => {
    ...
    vi.mocked(fs.readFileSync).mockReturnValue([
      JSON.stringify({ type: "session", id: "s1", timestamp: "2025-06-01T12:00:00Z", cwd: "/proj" }),
      JSON.stringify({ type: "message", id: "e1", message: { role: "user", content: "hi" } }),
      JSON.stringify({ type: "message", id: "e2", message: { role: "assistant", content: [] } }),
    ].join("\n"));
