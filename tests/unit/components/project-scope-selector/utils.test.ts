...
describe("normalize_path_segments", () => {
	it("splits path into segments, filtering empty ones", () => {
		expect(normalize_path_segments("/Users/me/project")).toEqual([
			"Users",
			"me",
			"project",
		]);
...
describe("build_workspace_groups", () => {
	it("groups projects by workspace label", () => {
		const projects = [
			make_project({
				name: "proj-a",
				path: "/Users/me/dev/org/proj-a",
				session_count: 5,
			}),
			make_project({
				name: "proj-b",
				path: "/Users/me/dev/org/proj-b",
				session_count: 10,
			}),
			make_project({
				name: "other",
				path: "/Users/me/work/co/other",
				session_count: 3,
			}),
		];
	...
	it("sorts groups by total session count descending", () => {
		const projects = [
			make_project({
				name: "a",
				path: "/Users/me/dev/org/a",
				session_count: 1,
			}),
			make_project({
				name: "b",
				path: "/Users/me/work/co/b",
				session_count: 100,
			}),
		];
	...
	it("sorts projects within groups by activity", () => {
		const projects = [
			make_project({
				name: "few",
				path: "/Users/me/dev/org/few",
				session_count: 2,
			}),
			make_project({
				name: "many",
				path: "/Users/me/dev/org/many",
				session_count: 20,
			}),
		];
