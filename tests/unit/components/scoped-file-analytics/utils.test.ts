...
describe("parse_excludes", () => {
	it("splits comma-separated values", () => {
		expect(parse_excludes("node_modules,dist,.git")).toEqual([
			"node_modules",
			"dist",
			".git",
		]);
...
describe("is_excluded", () => {
	it("returns true when path contains an exclude pattern", () => {
		expect(is_excluded("src/node_modules/pkg/index.js", ["node_modules"])).toBe(
			true,
		);
	});

	it("returns false when path contains no exclude patterns", () => {
		expect(is_excluded("src/lib/utils.ts", ["node_modules", "dist"])).toBe(
			false,
		);
	});
...
describe("recompute_directory_stats", () => {
	it("aggregates counts by parent directory", () => {
		const read = [
			{ name: "src/a.ts", count: 5 },
			{ name: "src/b.ts", count: 3 },
		];
...
describe("get_active_file_list", () => {
	...
	it("returns read files for 'read' tab", () => {
		expect(
			get_active_file_list("read", read_files, edit_files, write_files),
		).toBe(read_files);
	});

	it("returns edit files for 'edit' tab", () => {
		expect(
			get_active_file_list("edit", read_files, edit_files, write_files),
		).toBe(edit_files);
	});

	it("returns write files for 'write' tab", () => {
		expect(
			get_active_file_list("write", read_files, edit_files, write_files),
		).toBe(write_files);
	});
});
