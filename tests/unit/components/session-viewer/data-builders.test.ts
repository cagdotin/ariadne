...
function make_entry(overrides: Partial<SessionEntry> & { type: string }): SessionEntry {
  return {
    id: "entry-1",
    parentId: null,
...
describe("build_tool_call_map", () => {
  it("maps tool call IDs to name and arguments", () => {
    ...
    expect(map.size).toBe(1);
    expect(map.get("tc-1")).toEqual({ name: "read", arguments: { path: "/a.ts" } });
  });
...
describe("build_label_map", () => {
  ...
  it("handles multiple labels", () => {
    const entries: SessionEntry[] = [
      make_entry({ type: "label", targetId: "e1", label: "A" }) as unknown as SessionEntry,
      make_entry({ type: "label", targetId: "e2", label: "B" }) as unknown as SessionEntry,
    ];
  ...
  it("last label wins on duplicate targetId", () => {
    const entries: SessionEntry[] = [
      make_entry({ type: "label", targetId: "e1", label: "First" }) as unknown as SessionEntry,
      make_entry({ type: "label", targetId: "e1", label: "Second" }) as unknown as SessionEntry,
    ];
