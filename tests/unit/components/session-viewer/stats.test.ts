...
import type { SessionEntry, MessageEntry } from "@/components/session-viewer/types";
...
describe("compute_stats", () => {
  ...
  it("accumulates cost across messages", () => {
    const stats = compute_stats([
      ...
      assistant_msg({
        usage: {
          input: 100, output: 50,
          cost: { input: 0.03, output: 0.04, cacheRead: 0.003, cacheWrite: 0.004 },
        },
      }),
    ]);
  ...
  it("counts custom_message entries", () => {
    const stats = compute_stats([
      make_entry({ type: "custom_message", customType: "hook", content: "hi", display: true }),
    ]);
