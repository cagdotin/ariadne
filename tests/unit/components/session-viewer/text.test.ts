...
import { extract_text, has_text_content } from "@/components/session-viewer/utils/text";
import type { ContentBlock, TextContent } from "@/components/session-viewer/types";
...
describe("has_text_content", () => {
  ...
  it("returns false when content blocks have no text", () => {
    expect(has_text_content([{ type: "thinking", thinking: "..." }])).toBe(false);
  });
